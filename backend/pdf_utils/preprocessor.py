# backend/pdf_utils/preprocessor.py
...
def split_sections_with_layout(doc) -> dict:
    ...
    for span in doc.spans.get("layout", []):
        label = span.label_.upper() # etichetta in maiuscolo per confronto
        ...
        if label in ("SECTION_HEADER", "TITLE", "BOLD", "BOLD_CAPTION"):
            potential_title = span.text.strip().lower() # titolo in minuscolo per consistenza
            # evita titoli vuoti o troppo generici se non rappresentano una nuova sezione significativa
